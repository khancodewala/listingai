...
const PLAN_CONFIG = {
  free:   { color: 'text-slate-600',  bg: 'bg-slate-50',   bar: 'bg-slate-400',  badge: 'bg-slate-100 text-slate-600 border-slate-200' },
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      <div className="max-w-5xl mx-auto py-10 px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <a href="/generate" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-all text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                <div className={`h-1.5 rounded-full transition-all duration-700 ${pc.bar}`} style={{ width: `${100 - usagePercent}%` }}></div>
              </div>
              {limit !== Infinity && (
            ...
            <div className={`rounded-2xl p-5 shadow-sm border hover:shadow-md transition-shadow ${pc.bg} border-gray-100`}>
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                {plan === 'free' && (
                  <a href="/pricing" className="text-xs text-blue-600 font-semibold hover:underline">Upgrade →</a>
                )}
                {plan !== 'free' && (
                  <button onClick={() => setShowCancelModal(true)} className="text-xs text-red-400 hover:text-red-600 font-medium transition-colors">
                    Cancel
                  </button>
                )}
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { icon: '🏠', label: 'Listing Writer',    href: '/generate', color: 'hover:bg-blue-50 hover:border-blue-200' },
            { icon: '📱', label: 'Social Media',      href: '/generate', color: 'hover:bg-purple-50 hover:border-purple-200' },
            { icon: '✉️', label: 'Buyer Email',       href: '/generate', color: 'hover:bg-emerald-50 hover:border-emerald-200' },
            { icon: '📄', label: 'Contract Summary',  href: '/generate', color: 'hover:bg-amber-50 hover:border-amber-200' },
          ].map((action) => (
            <a key={action.label} href={action.href} className={`bg-white border border-gray-100 rounded-xl p-4 flex flex-col items-center gap-2 text-center transition-all hover:shadow-sm ${action.color}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-50">
              {generations.map((gen) => (
                <div key={gen.id} onClick={() => setSelectedGeneration(gen)} className="flex items-center justify-between px-6 py-4 hover:bg-slate-50/70 transition cursor-pointer group">
                  ...
                  <div className="flex items-center gap-3 ml-4 flex-shrink-0">
                    <span className="text-xs text-gray-400 whitespace-nowrap hidden sm:block">{formatDate(gen.created_at)}</span>
            ...
            {cancelSuccess ? (
              ...
              <>
                ...
                <div className="flex gap-3">
                  <button onClick={() => { setShowCancelModal(false); setCancelError('') }} disabled={cancelling} className="flex-1 py-2.5 px-4 border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-colors">
                    Keep My Plan
                  </button>
                  <button onClick={handleCancelSubscription} disabled={cancelling} className="flex-1 py-2.5 px-4 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                    {cancelling ? (
                      <>
